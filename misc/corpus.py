...
class Corpus:
    ...
    def embed_sentence(self, sentence: str, one_hot=True, pad: bool = True):
        sentence = f"{self.START_SYMBOL} {sentence} {self.END_SYMBOL}"
        tokens = self.tokenize(sentence)
        if pad:
            tokens = self.pad_sentence(tokens)
        # result = torch.LongTensor(self.max_sentence_length, self.vocab_size if one_hot else self.embed_size).zero_()
    ...
    def __call__(self, sentence, one_hot: bool = True):
